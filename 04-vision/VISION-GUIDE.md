...
*Part of the SUMMIT PUSH game package. This guide is subordinate to the Game Manual and the locked design specification; where any number here appears to conflict with the specification, the specification governs.*

This document defines the complete AprilTag layout for the SUMMIT PUSH field — **26 tags, tag family 36h11** — including exact tag-center coordinates, facing directions, panel geometry and placement, the machine-readable `apriltag-field-layout.json`, and software integration guidance for WPILib, PhotonVision, and Limelight, covering MultiTag pose estimation, auto-align transforms for CRAG scoring, FORECAST game-data handling, and simulation.
...
- The layout JSON (§4) is the machine-readable expression of this same geometry and **drives all simulation**: validate pipelines in PhotonVision or WPILib simulation against `apriltag-field-layout.json` (§6.8) rather than against any hand-measured layout.
...
- Field build tolerance on tag centers: **±0.25 in** in position. In the CAD model there is no such allowance: the model and `apriltag-field-layout.json` must agree **exactly** — MultiTag pose estimation is only as good as the layout's agreement with the JSON.
- CRAG tags sit at a **17.5-in center height**, HEADWALL lane tags at **12 in**, OUTFITTER tags at **52 in** on the alliance wall above each chute.

### 1.3 Why the CRAG tags are at 17.5 in, not 12 in

The CRAG tag height is set by occlusion, not by preference. A 9.0-in panel centered at 17.5 in spans Z 13.00–22.00, and its 8.125-in target spans Z 13.44–21.56.
...
| Low Socket tube, lowest point | Z = 22.27, the tube spanning 1.61–10.89 in outboard of the face | 0.71 in above the target — clear at every outboard station |
| ROPE COIL on a Low Peg | spans Z ≈ 26–36 | above the panel — clear |

At 12 in the shelf-face and inner-socket-face pairs would be blocked by any CACHE CRATE standing in the BASE DEPOT, which is a routine game state. HEADWALL lane tags stay at 12 in because that band is clear of every rung and keeps the whole panel at least 4.4 in behind the climbing plane.
...
> 19/20. The panel cannot simply be raised: at a 9.0-in panel, a centre of 17.75 puts the top
> edge at 22.25 against the Low Socket tube's 22.27, which is no clearance at all. Two ways
> out, and this is a design decision rather than a defect:
>
> 1. **Shrink the panel to 8.5 in and raise its centre to 17.75.** The panel then spans
>    13.50–22.00 — top edge unchanged, so the socket-tube and Shelf 1 clearances are
>    exactly as they are now — and the target moves to 13.6875–21.8125, restoring
>    **0.44 in** of CRATE margin. The cost is the white surround outside the target, which
>    drops from 0.4375 to 0.1875 in per side. This touches a CRITICAL vision dimension and
>    the tag layout JSON.
> 2. **Keep 9.0 at 17.5 and hold the build.** State the panel height tolerance as
>    **±0.15 in** rather than the ±0.25 assumed elsewhere, and check it at field setup
>    alongside the socket bore.
>
> Until that is settled the package publishes option 2's numbers, because they are what the
> current geometry gives.

**The upright-CELL case, and the camera-height rule it implies.** An O2 CELL is 14.0 in long, so a CELL stood on its end in the BASE DEPOT reaches 0.56 in above the bottom edge of a CRAG tag target. It is the only SUPPLY in the game that can. This matters for four of each CRAG's eight tags — the two on the SHELF FACE and the alliance-wall-side tag on each SOCKET FACE — because those are the three faces the DEPOT tray runs along. The other four (both PEG FACE tags and the peg-face-side SOCKET FACE tag on each side) have no tray in front of them at any height.

Whether the bottom edge is actually hidden depends on where the camera is. The obstruction is not a point: an O2 CELL's caps are **1.5-in domes** on a 5.0-in body, blended by a **1.0-in fillet** at each junction, so a CELL standing against the face presents a convex surface rising from Z = 12.47 where it touches the face to its apex of Z = 14.25 at 2.5 in out. (The fillet drops the shoulder from the 12.5 a bare dome would give, which is why the ranges below are a little more generous than the un-filleted part allows.) The sight line from the camera to the target's bottom edge descends as it approaches the tag, and it grazes that surface **on the flank nearer the face, not at the apex** — so a rule written against the apex alone understates the camera height needed, by as much as 5 in at long range. The clearances below are solved against the whole dome:
...
For a quick sanity check in the field, `h ≥ 13.44 + 0.56·D/2.5` is the apex-only form; treat it as a **lower bound that is 1–5 in optimistic**, never as the requirement.

Four consequences, none of which change the low-camera architecture of Section 5:

1. **Final approach is unaffected for a camera near the top of the recommended band.** At 20 in the whole target is visible from 15 in out — close-range alignment only. The as-built tray floor stands every SUPPLY 0.25 in higher than the carpet, which costs about a third of every range in this table. Below about 16 in a single camera sees the bottom edge clipped from almost any useful range whenever a CELL is stood on end in front of that face, so a design committed to one low camera should place it at **18 in or higher** and treat anything beyond about 10 in of range as localization only.
2. **Long range is already handled.** Section 5.2 treats long-range CRAG detections as localization input only, never as final-approach truth. That guidance exists for midfield traffic and it covers this case for free.
3. **Never rely on a single CRAG tag.** Every face carries a pair, and MultiTag (§6.4) over both — or over one CRAG face plus a HEADWALL or OUTFITTER tag — degrades gracefully when one tag is partly hidden. The optional second camera at 24–36 in (§5.2) clears the upright CELL out to 4 ft on its own.
4. **Simulate it.** Add a 5 × 14 in occluder standing at the DEPOT in front of one SHELF FACE tag (§6.8) and confirm the pose solution survives on the remaining tags.

A tag height that cleared an upright CELL outright would need its target bottom above 14.0 in — about 18.5 in center — but pushing the panel that high runs it into the Low Socket tube at 22.27 in and the Shelf 1 underside at 23.25 in. 17.5 in is the height at which the panel clears both of those with margin and clears every SUPPLY except the one that has to be balanced on its end to matter at all.
...
- **Yaw:** rotation about +Z, with **0° = facing +X** (toward the Red wall), measured counter-clockwise viewed from above. A tag's yaw is the direction its **face normal** points — the direction a camera must look *from* to see it head-on is the opposite.

There is **one canonical layout**. It is 180° rotationally symmetric about field center (324, 162): every Red tag *N* (14 ≤ N ≤ 26) is the 180° rotation of Blue tag *N − 13* about field center. Do **not** flip or re-origin the layout for the Red alliance — keep the blue origin always and transform *target* poses per current WPILib alliance-handling practice (see §6.1).
...
### 3.3 Geometry derivation (how the numbers were computed)

These values derive directly from the locked field geometry; entrants reproducing the field in CAD from the drawing set can verify them:
...
- **HEADWALL tags** are centered on each 48-in lane (Blue lane centers Y = 114, 162, 210) at **Z = 12 in**, mounted plumb on the truss lower crossbeam through a 15° wedge bracket, tag face plane **X = 39 (Blue) / X = 609 (Red)**. Plane P — the inclined climbing plane — is at X = 48 − Z·tan 15° for Blue, so a plumb panel at X = 39 clears P by 4.42 in normal at its top edge (Z = 16.5) and 6.75 in at its bottom edge (Z = 7.5), satisfying the ≥ 4.0 in structure clearance in FIELD-CAD-PACKAGE §4.1. Nothing about the panel enters the climbing volume.
- **Red = rotated Blue.** Every Red tag pose is the Blue tag pose (ID − 13) mapped by (x, y, yaw) → (648 − x, 324 − y, yaw + 180°). This is also a model-verification trick: measure the Blue tag poses in the CAD model, then check Red against the rotation.
...
The JSON and the inch table in §3 are generated from the same source geometry and **match exactly**; if either is edited, regenerate the other.

---

## 5. Camera Selection & Mounting

### 5.1 The low-tag rationale

The eight tags on each CRAG are mounted at a **17.5-in center height** and the HEADWALL lane tags at **12 in**, both deliberately low. The reasoning:

1. **Clear of scoring geometry, in every game state.** Nothing at those heights conflicts with shelves (24/42 in), socket rims (30/54/72 in), pegs (30/54/78 in), the DEPOT lip (4 in), or a crowned CACHE CRATE standing in the DEPOT (13.0 in, clearing the target by 0.44 in). No SUPPLY in a scoring position occludes a tag; the single exception anywhere on the FIELD is an O2 CELL stood on its end in the BASE DEPOT, which §1.3 budgets and prices in camera height.
2. **Visible at point-blank range.** The final 12 in of an approach is where alignment matters most. A low tag stays inside a low camera's vertical field of view all the way to bumper contact; a high tag exits the top of frame exactly when it is needed.
...
- **Localization camera (optional second camera):** mounted higher (24–36 in) with a wide field of view, angled slightly down or level, for mid-field pose estimation off OUTFITTER tags (52 in). Two cameras feeding one pose estimator is the standard high-end architecture.
- **Occlusion budget:** midfield traffic in the corridor will regularly block CRAG tags at long range. Autos should acquire the destination face's tag pair by roughly 6 ft out, and long-range detections should be treated as localization input only, never as final-approach truth.
- Global-shutter cameras at ≥50 fps with exposure locked low are strongly recommended; the white FORECAST LEDs, the alliance-color tier rings, and the SUMMIT BEACON will otherwise drive auto-exposure into smear.

> *Commentary:* A common failure at offseason events is a single high-mounted camera that sees everything at range and nothing at the moment of scoring. SUMMIT PUSH's tag layout is tuned for a low front camera. A single-camera design should mount it low.
...
Place the file in `src/main/deploy/`. The layout is already in the always-blue-origin convention — **do not call** `setOrigin(...)` **to flip it for Red**. Keep all robot poses blue-origin for the entire MATCH and mirror *target/waypoint* poses for the Red alliance in code. The 180° rotational symmetry makes that a one-line transform about field center:
...
2. Create an AprilTag pipeline: family **36h11**, tag size **0.1651 m** (6.5 in — the black square, per §1).
...
The layout puts **two co-planar tags** in view for every high-value action:

- Each CRAG face carries a **tag pair 28 in apart** (centers ±14 in). At final-approach distances both tags are in frame, and a two-tag PNP solve on a known baseline is far better conditioned than a single-tag solve: the pair fixes the target plane's yaw, which is exactly the degree of freedom a single planar tag resolves worst. Expect roughly 1 in and 1° of pose noise inside 5 ft with a calibrated camera. (A two-tag solve does not remove planar-target ambiguity in principle — it constrains it enough that the wrong branch is rejected by reprojection error at these baselines.)
- OUTFITTER and HEADWALL tags are single tags; treat them as localization beacons, not precision references. During the ENDGAME approach, the HEADWALL tag confirms *which lane* the robot is entering (tags 3/4/5 Blue, 16/17/18 Red); lane centering to ±2 in off a single 12-in tag at short range is realistic and sufficient, since the climb itself is a mechanical problem, not a visual one.
- MultiTag across *structures* (a CRAG pair plus an OUTFITTER tag in a wide-FOV frame) is a bonus, not a design assumption.

> *Example:* In AUTO, a Blue robot racing the CENTER CACHE localizes off tags 6/7 (Blue CRAG SHELF FACE) while driving up-field, snaps its pose estimate when both tags resolve at about 8 ft, and branches on the FORECAST (§6.7) to either the shelf approach (stay on 6/7) or a socket approach (transition to the 8/9 or 10/11 pair as it rounds the CRAG).
...
**SOCKET FACES** (tags 8/9 and 10/11 Blue; 21/22 and 23/24 Red — O2 CELLS, inserted from above). Each socket rim center is **directly above one tag of the pair, standing 8.0 in off the face plane**: the **Low Socket** (rim 30 in) above the alliance-wall-side tag (8, 10, 21, 23), the **Mid Socket** (rim 54 in) above the peg-face-side tag (9, 11, 22, 24):
...
A peg's tip is 7.07 in further out and 7.07 in higher than its root (10.0 in exposed at 45°).

**HEADWALL** (tags 3/4/5 Blue; 16/17/18 Red — tag at lane center, Z = 12, on the plumb tag plane at X = 39 / 609). The truss leans 15° toward the alliance wall, so rungs step **behind** the tag plane with height. Values are for approach planning only; the climb runs on superstructure sensing, not vision.
...
The lateral column is the same in **every lane** — the stagger alternates by rung, never by lane (FIELD-CAD-PACKAGE §4.1) — so one set of offsets serves all three lanes on both ALLIANCES, with +Y left of the lane centre from the approaching ROBOT's point of view. Red is the 180° rotation about field centre and carries the same signs relative to its own lane centres.

**OUTFITTER** (tags 1/2, 14/15 — tag centered above the chute): the chute opening is 30 in wide × 16 in tall with its sill at 24 in, so its center sits **20 in directly below the tag center**. Useful for auto-driving to the human-player feed station inside the OUTFITTER LANE.

> *Commentary:* Publish these transforms as constants, not magic numbers scattered through commands. One `FieldConstants` class holding the tag-pair midpoints and feature offsets — generated from the same geometry as the JSON — is the difference between retuning one file and retuning every auto if the field geometry is ever revised.

### 6.7 Reading the FORECAST (game data)

At T = 0 of AUTO, the FMS broadcasts the FORECAST as the game-specific message: **`"W"`** (WHITEOUT — PRIORITY SUPPLY: CACHE CRATES), **`"I"`** (ICEFALL — O2 CELLS), or **`"G"`** (GALE — ROPE COILS). The PRIORITY SUPPLY's AUTO placement points are doubled *and* ROPED UP requires two of it, so an auto that chases ROPED UP must genuinely branch three ways:
...
- The message may be **empty until the moment AUTO starts** — poll in `autonomousInit()` *and* re-check in the first iterations of `autonomousPeriodic()`; select the branch on first valid read and latch it. Never block waiting for it.
...
- **Offseason fallback:** without FMS, the FORECAST comes from the published card-draw procedure (Game Manual §4.3.1) and FIELD STAFF set the game data manually in the driver-station practice-match settings or via the offseason FMS tool. The code path is identical; test all three letters plus the empty string in every practice session.
- The ROUTE DECLARATION is **not** game data — it is chosen by the alliance during setup and known before the MATCH. Hard-code it per MATCH from the drive team's pick; only the FORECAST arrives at runtime.
...
- **PhotonVision sim (`photonlib` `VisionSystemSim`):** call `visionSim.addAprilTags(fieldLayout)` with the loaded layout, add a `PhotonCameraSim` with the real camera's calibration and the robot-to-camera transform, and the entire auto-align stack — including the low-camera framing of the 17.5-in CRAG tags and all three FORECAST branches — can be developed and regression-tested on the desktop.
- **Occlusion case worth simulating:** place a 5 × 14 in cylinder upright on the carpet immediately in front of one SHELF FACE tag — an O2 CELL stood on end in the BASE DEPOT, the one SUPPLY that reaches into a CRAG tag's target band (§1.3). Confirm that the pose estimator falls back to the face's other tag, or to MultiTag across faces, rather than emitting a bad single-tag pose.
