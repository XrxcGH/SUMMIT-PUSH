...
Build the SUMMIT PUSH field off-line and verify it.
    ...
    python organizers/source/featurescript/verify/run.py [--quick] [--only crags,tags,...] [--lit] [--stacked]
...
OpenCascade twin of the FeatureScript kernel (kernel_occ), then checks:
  ...
  * tags       — all 26 panels match participants/04-vision/apriltag-field-layout.json exactly
...
PKG = os.path.normpath(os.path.join(ROOT, "..", "..", ".."))        # the repository root
...
def check_tags(ctx, ns):
    errs = []
    with open(os.path.join(PKG, "participants", "04-vision", "apriltag-field-layout.json")) as fh:
        js = json.load(fh)
