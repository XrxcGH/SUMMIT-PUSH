...
`run-all.sh` first rebuilds `participants/01-game-manual/GAME-MANUAL.md` (with `organizers/source/manual/build.sh`) and the six drawing sheets (with `organizers/source/drawings/generate_drawings.py`), then runs `verify.py`, `consist.py`, `crossdoc.py`, `svg_collide.py` and `svg_geom.py`, in that order. Last, it runs `organizers/source/vision/make_layout.py --check`, which confirms that `participants/04-vision/apriltag-field-layout.json` matches the tag table in `participants/04-vision/VISION-GUIDE.md` §3. Each script can also be run on its own, from any directory, because it locates the package relative to its own path:

```bash
python organizers/source/verify/verify.py
```
...
The FeatureScript field generator in `organizers/source/featurescript/` has its own build and checks (see its `README.md` and `AUTHORING.md`). `run-all.sh` does not run them.
...
| `make_layout.py --check` | `apriltag-field-layout.json matches VISION-GUIDE.md §3` | `... DIFFERS FROM VISION-GUIDE.md §3`; regenerate the JSON with `python organizers/source/vision/make_layout.py`. |

`consist.py` also writes its report to `consist.txt` in this folder.
...
- `consist.py` does not scan `organizers/design/REVISION-LOG.md`, because the log quotes superseded text on purpose, or the `organizers/archive/concepts/` and `organizers/archive/research/` archives, which were written before the specification.
