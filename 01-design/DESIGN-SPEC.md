...
1. **CRAG APRON (placement protection; a line call).** A taped apron extends **36 in** out from the SHELF FACE and the PEG FACE and **20 in** out from each SOCKET FACE, with 20-in-radius arcs sweeping the four corners. An opponent may not contact a robot whose bumpers intersect its own alliance's apron (MAJOR FOUL; + YELLOW CARD if contact causes a robot then extended above 60 in to tip). The 68-in inter-CRAG corridor (Y 128–196), the nine staged CENTER CACHE marks, and all transit lanes remain open to defense; the outer 20 in at each end of the CENTER CACHE band lie inside an APRON. Defense is otherwise legal everywhere except the opponent's OUTFITTER LANES and the opponent's HEADWALL ZONE during ENDGAME.
...
5. **O2 socket tolerance published:** socket ID **6.50 in ± 0.125 in**, the only strictly toleranced field dimension, with a callout in the manual. The field CAD model and its drawing set must hold it.
...
- **Lanes and zone:** one robot per lane. HEADWALL ZONE = the taped BASECAMP area. An opponent contacting a robot in this zone during ENDGAME commits a MAJOR FOUL (+ YELLOW if a hanging robot falls).
...
**Full capacity** is 7 SCORED positions for every ROUTE. The three requirement sets are distinct and cost 12 / 12 / 11 CRAG positions in total (LOW / MID / HIGH), which is 4 / 4 / 3 beyond the three CAMPS that every ROUTE needs anyway. HIGH asks for one position fewer because the one it asks for is the hardest on the CRAG:
...
- Contact rules are written by OUTCOME (damage / functional impairment / tipping) rather than by geometry. The CRAG APRON and the ENDGAME HEADWALL ZONE are the two intended line calls.
...
- Tone: third-person competition-manual voice. No sponsor, no team identity, no first-person plural, no marketing register. Defined terms, blue-box examples, and gray commentary boxes are rendered as blockquotes.
