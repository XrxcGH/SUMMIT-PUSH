...
This log records every substantive change to the SUMMIT PUSH package from v1.0 to the current v2.2, with the reason for each change and, where it matters, the arithmetic behind it. It is a historical record. Entries describe the package as it stood when each change was made and quote superseded values on purpose; the current values are in `01-design/DESIGN-SPEC.md` and the documents that follow it.

The log is divided into lettered sections by revision:

| Sections | Revision | Contents |
|---|---|---|
| A–I | v1.0 → v2.0 | The first full audit: editorial and identity changes (A), blocking geometry defects (B), other geometry corrections (C), scoring and balance (D), rules corrections and additions (E–G), new files (H), and the first verification script (I). |
...
File paths are given as they are now. The verification suite lives in `verify/` and is described in `verify/README.md`. `_PROGRESS.md`, the working status tracker named in H5, L22 and M18, was removed before the public release.
...
| A2 | Removed every reference to a specific team, including the tie-breaker example, the CADathon note, the judging "buildable by X's shop" line, and the README credits. | So that any program can use the release as a training exercise. | spec, sections 01/03/06, vision guide, brief, README |
...
| A5 | Tightened the story section (§2.1 "The Setting"): removed the closing slogan and the hype adjectives, and kept the setting. | Real season manuals carry a short thematic passage; they do not carry taglines. | section 01 |
...
### B1 — Unsatisfiable BASE DEPOT scoring test

**v1.0:** a SUPPLY was SCORED in the BASE DEPOT only when "entirely below the plane of the top of the DEPOT lip (Z = 4 in)."

**Problem:** the CACHE CRATE is a 12.0-in cube (13.0 in across its pillowed faces) and the O2 CELL is 5.0 in in diameter. Neither can be entirely below 4 in in any orientation. Only the ROPE COIL could be SCORED in a DEPOT, which contradicted the "accepts any piece in any orientation" text, the `any` row of every scoring table, the Regional CAMP I substitution, the G503 example, and the whole BASECAMP BOT reference design.
...
### B4 — OUTFITTER chute too small for a CACHE CRATE
...
### B5 — BUMPER ZONE shallower than a legal BUMPER
...
### B7 — Ineffective lateral rung stagger

**v1.0:** rungs 30.0 in long, staggered ±6.0 in.

**Problem:** LEDGE spanned lane-centerline −21 to +9 and CAMP spanned −9 to +21, so an 18-in-wide common band engaged all three rungs with zero lateral motion. A straight telescoping hook followed the rung line, contradicting the manual's own claim.

**v2.0:** rungs **20.0 in** long, staggered **±12.0 in**. LEDGE spans −22 to −2 and CAMP spans +2 to +22, so **no lateral position engages two successive rungs**. Rungs stay 2 in inside their lane and the minimum inter-lane gap is 4.0 in. The stagger convention is now published per lane, including the Red mirror.
...
| C3 | CRAG APRON offset **36 in everywhere, trimmed at the CENTER CACHE band** → **36 in on the SHELF and PEG faces, 20 in on the SOCKET faces, corners filleted R20, no trim**. | The trim rectangle coincided with the inter-crag corridor, and each CRAG's field-center-facing SOCKET FACE apron lay entirely inside it, so one of the two socket faces on every CRAG had no APRON protection, and nothing said so. Two 36-in aprons also could not coexist with the staged CENTER CACHE inside a 108-in corridor. At 20 in the aprons occupy Y 108–128 and 196–216, leaving a 68-in open corridor that contains the whole cache (Y 132–192). The corner radius drops from 36 to 20 so the fillet is tangent to both offset lines; a 36-in arc cannot meet a 20-in offset. G407 loses its band-exclusion clause and becomes a clean line call. |
...
### D1 — Uniform placement values by tier

**v1.0 (TELEOP):** Shelf 1 = 3, Shelf 2 = 5, Low Socket = 4, Mid Socket = 7, Summit Socket = 10, Low Peg = 4, Mid Peg = 6, High Peg = 9.

**Problem:** CACHE CRATES were worth 24 points across six positions (4.00 per piece) against O2's 32 across five (6.40) and rope's 38 across six (6.33). Crates are the largest and heaviest piece and demand the widest intake and a 42-in lift, and a Shelf 1 placement was worth barely more per second than dumping in the DEPOT. Playing the CRATE FREIGHTER role cost an ALLIANCE points.

**v2.0:** **Low tier 7 AUTO / 4 TELEOP · Mid tier 10 / 7 · High tier 13 / 10 · BASE DEPOT 4 / 2.** Value is set by tier alone, whatever the piece type.
...
### D3 — FORECAST with no branch value

**Problem:** doubling one type's AUTO placement did not change what any ROBOT should do: the optimal AUTO target set was identical in all three branches, with a 1–2 point spread. The mechanic added points without adding a decision.

**v2.0:** ROPED UP now requires **≥5 SUPPLIES including ≥2 of the PRIORITY SUPPLY and ≥1 of each other type** (was ≥4 including ≥1 of each). Four of the five are pinned by type (two of the priority type plus one of each other), and which type is doubled is unknown until T = 0, so an ALLIANCE chasing the bonus must prepare and agree on three routines. Branch stake is now the +10 bonus plus the doubled value of the extra priority piece.

*Files:* spec §4, sections 01 §2.2/§2.4, 03 §4.3.4.

### D4 — ASCENT threshold

Championship **62 → 60**. At 62, an ALLIANCE whose three ROBOTS all completed a 54-in CAMP RUNG climb (3 × 20 = 60) missed the RP by two points, while one summit, one camp and one ledge climb met it exactly. In practice the threshold required two SUMMIT RUNG climbs. Regional (32 = CAMP + LEDGE) and DCMP (52 = CAMP + CAMP + LEDGE) are unchanged and consistent.

### D5 — ROPED UP earnable in TELEOP
...
| E10 | G408 | PINNED now requires contact with "a ROBOT **other than the pinning ROBOT**"; separation reduced 10 ft → **6 ft**. | The old qualifier was always satisfied (the pinning ROBOT is itself another ROBOT), so every sustained push was a pin. A 10-ft separation, a third of the field width, is hard to judge by eye. |
| E11 | G501 | Violation line adds: SUPPLIES CONTROLLED in violation earn no points and do not count toward SUPPLY LINE. | Foul points cannot reach a bonus-RP threshold, so bulldozing eight loose SUPPLIES into the DEPOT cost 18 foul points and bought 16 points plus a third of a ranking point, which is rational in any decided MATCH. |
...
| E20 | G102 / §3.5 | Both now say the same thing: HUMAN PLAYERS may break the plane of the **chute opening** and no further. | §3.5 said "the plane of the alliance wall," which the chute pierces. G102 permitted what §3.5 forbade, and a MAJOR FOUL depended on which text a referee applied. |
...
| F5 | **§4.3.5 Designing an AUTO** and **§4.4.5 Cycle reference**: published FIELD distances. | So that points-per-cycle models can be built from the published geometry, as the judging rubric asks. |
...
A cross-check script was written to re-derive, from the documents themselves:
...
At v2.0 it reported zero failures. The manual's internal cross-references (89 rule IDs, 86 numbered sections) also resolved with no dangling targets, and the glossary was reported to cover every ALL-CAPS term used in the body.

---

## J. v2.1 — second-pass corrections

These changes came out of a re-audit of v2.0 itself. The first audit had only
seen v1.0, so every v2.0 change was unverified.
...
| J2 | **G501** violation rewritten. The clause voiding points and SUPPLY LINE credit for over-CONTROLLED SUPPLIES *for the remainder of the MATCH* was replaced by an escalation: CONTROL of four or more SUPPLIES at once, or herding SUPPLIES as a group toward a scoring location, is a **MAJOR FOUL per extra SUPPLY**. | The original consequence required referees to track individual rubber rings and cardboard crates for two minutes after a call, which referees could not do in practice. The escalation prices bulldozing out of the game with a single count at the moment of the call: eight herded SUPPLIES is six MAJOR FOULS (48 points) against 16 points of cargo. | section 04 |
| J3 | **G502**'s launch exception re-drawn. *"CRAG APRON on the SHELF FACE side, within 36 in of the SHELF FACE or of the DEPOT's corner wraps"* → *"the portion of its own ALLIANCE's CRAG APRON that lies on the same side of the FIELD centerline as its own SHELF FACE."* | The old region was not a taped area and could not be judged from the referee platform: it measured 36 in from a curved corner wrap. Both CRAGS straddle X = 324, so the new region is the half of the APRON on the ROBOT's SHELF FACE side of the centerline. It is bounded by two lines the FIELD already carries, and it contains the SHELF FACE, the tray, and both corner wraps. | section 04, glossary |
...
| J8 | **§4.3.5** LEAVE row corrected from "~36 in of travel" to "24 in, plus half the ROBOT's BUMPER length". | The published figure assumed a particular ROBOT length; LEAVE is a BUMPER test, so the distance depends on the ROBOT. | section 03 |
...
| J14 | Occlusion budget corrected. The CACHE CRATE was budgeted at 12.0 in rather than its crowned 12.5, and the one SUPPLY that reaches into a CRAG tag's target band, an **O2 CELL stood on its end in the BASE DEPOT** (14.0 in against a target bottom of 13.44), was missing. §1.3 now carries the case, the camera-height rule `h ≥ 13.44 + 0.56·D/d`, and a range table; §5.1's "SCORED pieces never occlude a tag" was false as written and is corrected; §5.2 raises the single-camera floor to 18 in; §6.8 adds the occluder to the simulation recipe. | An occlusion budget that omits a case is worse than none, because it is relied on. | vision guide, verification suite |

---

## K. v2.1 — findings from the adversarial re-audit of v2.0

A ten-auditor adversarial pass was run against v2.0 itself, and each finding was
then given to independent verifiers instructed to refute it. This section lists
the blockers and the first wave of confirmed findings, with the action taken on
each; section L lists the rest. Four of the confirmed findings were blockers,
and three of those had been introduced by the v2.0 revision.

### Blockers

| # | Change | Why |
|---|---|---|
| K1 | **G416 changed in two ways.** The zone test became a **plane test** against the **CLIMB LINE** at X = 48 (Blue) / X = 600 (Red), across the full width of the FIELD, and the exemption for rung support became **current** rather than lifetime. | Two independent holes. (a) BASECAMP spans only Y 90–234, but lane 1's SUMMIT RUNG reaches Y 92, so a ROBOT parked beside BASECAMP at Y ≈ 75 had every BUMPER outside the zone while sitting directly under that rung, 5 in of extension from its end. (b) The J4 carve-out said "has already been supported solely by a rung **at any point in the MATCH**": a permanent exemption earned with one brief unweighting on the LEDGE RUNG, after which the same vertical mast could take the SUMMIT RUNG straight up. Either hole reopened the play B6 created the rule to close. The CAD package's climb-reach analysis had assumed the plane test all along, so the rule was the outlier. |
| K2 | **G416's trigger changed from "supported by the carpet" to BUMPER position plus rung support.** | A ROBOT that unweighted itself on the truss lower chord, or on a partner, was not "supported by the carpet" and so fell outside the rule entirely, at the cost of at most one **G202** foul against a 30-point climb. |
| K3 | **Mirrored rectangles in the plan sheets anchored at the wrong edge.** `s.rect(..., fy(Y(234)), ..., SC*144)` reads correctly for Blue and, under the 180° mirror, anchors at the band's *low* Y edge and runs 144 in further down. | The Red BASECAMP, the Red HEADWALL truss band on two sheets, and both Red OUTFITTER lanes and chute openings were drawn 144 in / 36 in / 24 in south of where they belong (54 in of several shapes hung off the carpet), while the lane divider lines and text labels on the same sheets were drawn at the correct Y. Fixed by anchoring every mirrored rect at `min(fy(a), fy(b))`. |
| K4 | **Socket tubes drawn leaning the wrong way in both CRAG elevations.** | VIEW A and VIEW B rotated the tubes so all five sockets leaned *inward* at the open end, the opposite of the locked "tilting outward, open end up," and of details D1/D2 on the same sheet. The side-socket bottoms were drawn 11.50 in outboard instead of 4.50, the Summit Socket's 9.81 instead of 6.19. Sign flipped; the decoded values now read 4.50 and 6.19 exactly. |
...
| K6 | **G412**: blocking or displacing a climb is now an **additional MAJOR FOUL and a YELLOW CARD**, and the 5-second accrual cap is removed. | One incursion cost the fouling ALLIANCE 8 points and cost the victim an 18-point climb downgrade plus its ASCENT RP, since §4.8 bars foul points from bonus-RP thresholds. Denying a SUMMIT climb was worth about +10 net. Removing the cap also resolves G413 being priced at twice G412 for a lesser offense. |
| K7 | **G412's forced-position exception** no longer covers a ROBOT that invited the contact, and a ROBOT of the ALLIANCE that owns the zone neither violates the rule nor triggers the exception by contacting an opponent already inside it. **G205** gains clause (c) for reverse baiting. | An opponent could stop 2 in short of the HEADWALL ZONE tape astride the lane, be pushed across it, and collect a MAJOR FOUL from the ROBOT trying to reach its own truss, so the protection ran the wrong way. |
...
| K12 | **PARK** row and glossary entry narrowed to HEADWALL structure *other than a rung*, and **G416** gains "a ROBOT that earns only PARK is not penalized under this rule." | The PARK row permitted contact that G416 forbade, on an identical fact pattern; the LEDGE RUNG hangs 7.8 in inside the tape, so a parking ROBOT routinely brushes it. |
...
| K14 | **§4.9** score finality gains a backstop: 10 minutes after the final buzzer where there is no next MATCH, extendable once by announcement. | "No later than the start of the next MATCH played on that FIELD" never fires for the last MATCH of a phase, a day, or the event. That left the whole G-rule set, the question box, and G504 restorations open-ended, since all three are scoped to "before the score is FINAL." |
...
| K16 | **R207** rewritten: the banned block display is white **for the FORECAST** and ALLIANCE-colored **for the ROUTE**, the closing permission is qualified against one-, two-, and three-block arrangements, and the amber **R706** ROBOT SIGNAL LIGHT is expressly exempt. | The ROUTE indication is ALLIANCE-colored, not white, so the ban was written against the wrong color while expressly permitting the lookalike. And R207's "could be mistaken for a SUPPLY" bullet, read literally against the amber ROPE COIL (`#D9A441`), failed every ROBOT that complied with R706's mandatory amber light. |
...
| K20 | **BASE DEPOT tray closed into a continuous U:** the two 16 × 16 in outer corner squares are now dimensioned and drawn. | The main leg and the two arms met only at two points, so the "open-topped U in plan" was three disconnected rectangles. Open-from-above area rises to four 16 × 16 in openings. |
| K21 | **VIEW D shelf footprint** was drawn 14 in *into* the tower (`K * (SHELF_DEPTH - SHELF_DEPTH)`, a faulty edit that evaluated to zero); now cantilevered outward over the channel. | The view contradicted its own note, detail D4 on the same sheet, and the open-to-sky check. |
...
| K28 | **§3.2 worked example** corrected: the −Y SOCKET FACE points *into* the inter-crag corridor, toward field center, not away from it. | The sentence teaches which face a ROBOT is looking at, and the next sentence depends on the corridor geometry. |
...
| # | Change | Why |
|---|---|---|
| L1 | **Rung stagger no longer alternates by lane.** Every lane now carries LEDGE −12.0 · CAMP +12.0 · SUMMIT −12.0. | Alternating the sign per lane put rungs at the same height in adjacent lanes **24 in apart center-to-center** (closer than two hanging ROBOTS are wide at the 120-in frame-perimeter limit), which made "all three ROBOTS can climb simultaneously with zero traffic conflict" false and an inadvertent **G415** violation the normal case. Uniform stagger puts them 48 in apart, 28 in end to end, and preserves the property the stagger exists for: no lateral position engages two successive rungs *within* a lane. |
...
| L6 | **DEPOT capacity re-derived** from "~12 pieces" to about **8 CACHE CRATES**, or about **12 SUPPLIES** mixed. | B1 made DEPOT scoring a single-layer floor-support test, and the capacity was not re-derived. Twelve crowned crates need 2028 in² against 1792 in² of tray, so the published figure was unattainable for the piece most likely to be counted. |
| L7 | **The 20-in SOCKET FACE apron is shallower than the legal reach**, and this is now published. Protection there requires engaging the rim with 15 in of extension or less; at 18 in a ROBOT's BUMPERS sit 3 in outside its own tape. | C3 justified the 20-in offset on corridor packing and never checked it against **R105**. The consequence is real and affects one side only, so it is disclosed; changing the offset would break the corridor again. |
...
| L10 | Drawing corrections: VIEW B was labeled "+Y face shown" while drawing the −Y face; detail D1's toleranced **ID 6.50 ± 0.125** was built from OD endpoints and measured 6.68; the Red tag-label token swap was a one-line fragment of a three-step rotation. | Each is a drawing that says something the ledger does not. |
...
| L15 | **§8.6** gained three fixes: a ROBOT hanging at a stoppage is recorded and credited; the stop/replay contradiction is resolved in favor of the Replays paragraph; and TIMEOUTS are one per Playoff bracket plus one for the Finals, replacing an undefined "series". | Each was a gap in a section added by this revision. |
...
| L19 | **R805 / R808**: relief valve set between 125 and 130 psi, compressor stopping at ≤115 psi. **R703** no longer offers the compressor a power path **R807** forbids. **R707** permits the FMS connection it requires. **R207** bans the right colors and exempts the mandatory amber RSL. | Read literally, four rules could not be satisfied, and three of them had been added by this revision. |
...
| L24 | Glossary gained **CLIMB LINE**, **PLANE P**, and **DCMP**; **ALLIANCE ROLE** is now used in the body; **PARK** and **BASE DEPOT** re-worded to match the rules they summarize. | A defined term that appears nowhere, and a body term with no entry, are the same defect from opposite ends. |

### Findings not acted on

Twenty MAJOR and twenty-five MINOR findings were refuted by their verifiers, and
no action was taken on them. Two of the four blockers were refuted on their own
terms but had already been fixed by the G416 rewrite that a third, confirmed
finding required, so the fix stands either way.

---

## M. v2.2 — third pass: audit of the v2.1 fixes

The changes in sections J–L had not been audited when they were made. A third
adversarial pass was run against v2.1 with eight lenses and two refuters per
finding, together with an independent re-derivation of the balance model and a
read-only red-team review of the rewritten rules. Most of what it found fell into
two patterns:

1. **Fixes that reached the rule but not every restatement of it.** Five
   separate defects were the same L1 change failing to reach a table, a caption
   or a note that also published the geometry.
2. **Checks that could not fail.** Four checks in `verify/` were tautologies,
   one was computed on a superseded dimension, and the scanner skipped the
   drawing set.
...
| M7 | The glossary-coverage check anchored on the **GLOSSARY heading** rather than on `\| **A-STOP**`, headword capture widened to qualified rows and parenthetical aliases, and morphological variants normalized. | The glossary is alphabetized ignoring punctuation, so A-STOP sits *after* ALLIANCE, and anchoring there silently dropped the first five headwords. Between that, rows like "**APRON** (CRAG APRON)" that the regex could not match, and every plural counted as a miss, the check reported 61 false positives and was therefore ignored. It now reports **0**, and fails when a real entry is removed. |
...
| M11 | **R207's** closing permission narrowed to a countable block pattern. | Read literally, K16's qualifier ("provided it is not arranged as one, two, or three discrete lit blocks") banned a single alliance-colored LED bar, so the sentence that exists to permit ordinary lighting prohibited the commonest form of it, with INSPECTION failure as the consequence. |
...
| M14 | Five stale drawing notes corrected: the front elevation's lane-2 caption (which contradicted the line directly above it and the geometry the sheet draws), the 4.0-in inter-lane gap, D2's nominal-crate clearance and the missing mast constraint over Shelf 2's center slot, D3's 47° coil tilt, and the RUNG DETAIL's "full hook wrap" against K23's 16-in wrappable length. | The sheets are what entrants model from. |
| M15 | The **FIELD centerline** added to the manual's markings table, the **CLIMB LINE** taped across the full field width, and the tier rings' 78-in band reconciled with its own center height. | G402 and G502 are line calls against the centerline, which the referee-facing table did not list; G416 is a line call against a plane the tape only covered over Y 90–234; and a 1.0-in band spanning Z 77.0–78.0 is centered on 77.5, not the 78 the spec locks. The 78 ring is now explicitly hung with its top edge on the lantern joint. |
| M16 | The **ROUTE declaration EV model published**, and the false "no declaration is cheaper than another" claim removed. | The spec asserted that no ROUTE dominates without showing it, while J1 had made the incremental costs 4 / 4 / 3. Re-derived: declaring pays 53 / 55 / 61 when the declared CAMP lands and 25 / 21 / 16 when it does not, so HIGH beats LOW at p ≈ 0.79 and each declaration is optimal for a real capability profile. That is the property the uplifts were tuned for, and the published model now shows it. |
...
| M24 | The BASECAMP BOT's first graduation rung corrected from "add a wrist" to **"add a wrist, a 30-in lift, and about 12 in of extension"**, with the Low Socket standoff published alongside it. | The reference design is a fixed-height claw with no lift and no extension, so a wrist supplies neither of the two things a Low Socket needs. The rookie-facing ladder also omitted the J12/K18 fact the package had taken care to establish: the DEPOT's corner arm holds BUMPERS 16.75 in off the SOCKET FACE, making that rim 11.75 in of extension away rather than the Mid Socket's 5.0. |
| M25 | The **direct LEDGE → SUMMIT climb** documented as a legal second route, replacing three claims that the CAMP step is mandatory. | L1's uniform stagger alternates with period two, so the LEDGE and SUMMIT RUNGS share the −12.0 offset and a climber with two hook sets at one lateral position can go straight from one to the other: 48 in up, 12.86 in back, 49.69 in measured in plane P, against 24.85 in for a single traversal step. The geometry makes this unavoidable: a 20.0-in rung in a 48-in lane admits only two non-overlapping lateral positions, so two of the three rungs must share one. It also needs a harder mechanism (twice the reach in one move), so it stands as a legitimate alternative route. §3.4, inspection checklist item 10 and the HEADWALL sheet now say so instead of demanding evidence of a traversal the rules do not require. |
| M26 | **G416's tail re-scoped from the clock to the rung**, third and final attempt: the exemption now covers a ROBOT "still touching a rung it took hold of while so supported within the preceding 5 seconds". | M1's "has not driven" condition did not close the touch-and-go, for a reason that is not obvious. Hanging from the LEDGE RUNG settles the chassis under the hook at X = 40.16, so a ROBOT at the 120-in FRAME PERIMETER limit already spans X 25.16–55.16, which contains the SUMMIT RUNG at X = 27.30. It never has to drive: unweight for an instant, let the hang center it, lower straight down, mast up with zero horizontal extension. Tying the tail to the rung closes the loophole. A ROBOT that falls or lowers mid-traversal is still touching a rung it had hold of, so it stays protected; a ROBOT that releases, returns to the carpet and reaches for a different rung is not; and a ROBOT with the reach to take the SUMMIT RUNG *while still hanging* is doing the legal direct climb of M25. |

### Completeness review

The audit's last stage asked one agent what the eight lenses had missed. It found
thirteen more. The main pattern it named was the first one listed at the start of
section M: while fixing restatement defects, the pass had made the same error four
more times.

| # | Change | Why |
|---|---|---|
| M27 | The **K23 rung-bracket exception** carried into DESIGN-SPEC §3 and manual §3.4. | It had reached the CAD package, the glossary and the drawing sheet, but not the two documents that outrank them. Both still said "all truss structure ≥ 4.0 in behind plane P", and the manual added the false consequence "so every rung offers full hook wrap". Wrap is clear over 16.0 in of a 20.0-in rung. A hook placed within 2 in of a rung end collides with a bracket the governing text says is not there. |
| M28 | The **CLIMB LINE added to the manual's markings table**. | M15 taped it in the CAD package and the render checklist. The manual, whose §3 is "the authoritative physical description of the ARENA" and whose §3.2 opens "all zone boundaries are marked with tape", described the CLIMB LINE only as a *plane*, never as a marking. A field built from the governing document has no tape at X = 48 outside Y 90–234, which is the strip M15 was written for. |
| M29 | **G413** given M19's cap and immobility exemption. | M19 fixed G412 and left its sibling untouched, re-creating the pricing inversion K6 had cited as its reason for removing the cap: the lesser offense now carried the larger accrual, uncapped, and G413 is not even scoped to the ENDGAME. |
| M30 | **Sheet 1's tape key** completed from four markings to six, the CLIMB LINE drawn dashed outside BASECAMP, and X = 324 redrawn as white tape broken at the CRAG footprints. | Drawing 1 of 6 is the sheet a field is taped from, and it omitted the two markings most recently added, which are the two that carry rule calls. What it drew at X = 324 was a muted dash-dot *construction* line, unbroken, in the wrong color. A builder taping from sheet 1 laid neither line. |
| M31 | The **ROUTE EV crossover corrected** from a bare "p = 0.79" to both figures with their conditions. | M16's own formulas (25 + 28p, 21 + 34p, 16 + 45p) cross at p = 9/17 ≈ 0.53 and p = 5/11 ≈ 0.45 at *equal* probability. The published 0.79 silently froze p at 0.95 for CAMP I while varying it for HIGH CAMP. That is a different and more realistic question, but not the one the printed derivation asks. Both are now stated with what they assume. The "strong" profile was also an exact tie at 51.60, so MID never strictly won in any published row; it is now .92 / .90 / .55, where it wins outright. |
| M32 | The **camera-height rule re-solved against the domed cap**. | K14's rule modeled the upright O2 CELL as a point at its 14.0-in apex. The caps are 1.5-in domes, and the sight line grazes the **flank nearer the face**, not the apex, so the rule understated the required height by up to 5 in and every range in the table was 20–25% optimistic. Corrected: a camera at 20 in clears to 22 in, not 29; at 30 in to 56, not 74. The apex form survives only as an explicitly labeled optimistic lower bound. |
| M33 | Two more checks in the suite that could not fail: the **CAMP row's sign test** accepted either sign (stripping '+' from '+12.0' leaves '12.0', a substring of a minus-signed cell, so flipping the one row the whole stagger property depends on left the suite green), and the **stale-term scan was case-sensitive**, missing a CRITICAL ledger row still labeled "Corner wraps". Both fixed and both re-proved by injecting the regression. | |
| M34 | The **HEADWALL crossbeam, wedge and tag panel** no longer occupy the same volume. | L12's three (ref) bodies interfered: the beam's field-side face at X = 40.64 against a panel back at X = 38.75 and a wedge filling the space between them. The wedge thicknesses were self-consistent, which is what exposed the error: they were computed against the clearance *surface*, then attributed to the front face of a 2-in-deep beam. The beam is now notched at the three lane-tag stations. |
...
| M39 | **`crossdoc.py` added as a fifth check**, and the stale-value scan made case-insensitive. | `consist.py` asks whether a superseded value survives. Nothing asked the opposite question: whether a value that must appear in several documents appears in all of them. A fix applied correctly in one place and never written in another leaves no stale text to find; the absence reads as clean. The new check caught two on its first run. The case-sensitivity fix was one the audit had raised and whose patch had silently aborted on an earlier assertion, so a CRITICAL ledger row labeled "Corner wraps" had been passing a scan written to catch it. |

### First CAD build

The CACHE CRATE was modeled in Onshape: 12-in cube, 0.5-in pillow from the center
of each face, 1.0-in fillets on all twelve edges. That matches §9.1, and building
it exposed one defect the drawings had carried since v1.0.

| # | Change | Why |
|---|---|---|
| M40 | **Shelf-slot clearance re-derived against the crown**, replacing the nominal-cube figure. | §9.1 and the SUPPLIES sheet both published "1.0 in of clearance per side" from the 12.0-in nominal cube. A CRATE rests on its bottom crown, so the cube's bottom plane sits 0.5 in above the shelf and each side face reaches its full 0.5-in bulge **6.5 in** up. The binding width is at the top of the 2.0-in fences, where the crate is **12.44 in** across, leaving **0.78 in per side**, a 22% smaller placement tolerance than published, and the number a scoring mechanism is designed against. The 13.0-in crown sits 4.5 in above the fences and touches nothing; crates in adjacent slots clear each other by 2.5 in. Now in the CAD package, the manual's §3.3.1, the brief's architecture section and the drawing sheet, with the crown profile locked into the verification suite. |
...
| M43 | **ROPE COIL confirmed to spec, and its construction recorded.** | All three published numbers reproduce from the single revolve: a 2.5-dia profile circle whose outer edge is 5.0 from the axis puts its center at r = 3.75, giving OD 10.0, ID 5.0 and a 2.5 tube. The derived 47.87° tilt bound reproduces too. The suite now derives OD and ID from the profile rather than asserting them separately, and §2.5 states the **2.9° margin** between that bound and the 45° a vertical hang needs, the number that makes a 45° peg capture a coil rather than let it balance. Nothing needed changing. |
| M44 | **BASE DEPOT as built.** Plan confirmed exactly (48-in notch + two 16-in arms = the 80-in outer run). Recorded: **tray floor top at Z = 0.25** above the carpet, lip top 4.0 above the carpet and therefore **3.75 above the floor**, R0.25 on every outside face, and `depot` recolored **#7F7F7F** from #B8A888. | The package had the tray floor "flush-shimmed to carpet" and every occlusion figure assumed SUPPLIES rest at Z = 0. A tray laid on the carpet stands everything in it 0.25 in higher, which propagates: a crowned CACHE CRATE now apexes at **13.25** against a 13.4375 target bottom (**0.19 in**, down from 0.44), an upright O2 CELL at **14.25** (0.81 into the band, up from 0.56), and every camera range in VISION-GUIDE §1.3 loses about a third (at 20 in, 15 in of range rather than 24). The D4 depot section was drawing the CRATE resting at Z = 0 and 12.0 tall; it now sits on the floor at its crowned 13.0. |
| M45 | **Open decision recorded:** the 0.19-in CRATE-to-tag margin is smaller than the panel's own build tolerance. | A field built 0.2 in low would let a CRATE clip the bottom of tags 6/7 and 19/20. Raising the panel alone does not work: a 9.0-in panel centered at 17.75 puts its top edge at 22.25 against the Low Socket tube's 22.27. §1.3 now carries both ways out: shrink the panel to **8.5 in and raise the center to 17.75**, which holds the top edge at 22.00 so every upper clearance is unchanged and restores **0.44 in** of margin at the cost of white surround (0.4375 → 0.1875 per side); or keep 9.0 at 17.5 and state the panel height tolerance as **±0.15 in**. The package publishes the second, because it is what the current geometry gives. This is a design decision, so it is flagged rather than made. |

### Deferred style-guide findings

This pass also fixed all twelve style-guide findings that the second audit
confirmed and that had been deferred: the `attr()` that cannot resolve in a
page-margin box, the footer string the specified mechanism cannot produce, the
full-bleed band on a page declared to need no bleed, the 60-vs-148-page
contradiction, the alignment-row claim that no table satisfies, five contrast
ratios that do not recompute, the italic-versus-bold rule headline, the
production recipe's unshipped files, and the drawing sheets specified at a
measure where their type sets at 2 pt.
...
| # | Change | Why |
|---|---|---|
| N6 | §4.4.4 now says a ROBOT may not deliberately eject a SUPPLY (**G507**); it had named HUMAN PLAYERS too, which no rule covers. §4.2.3 now has the ROUTE reported to the HEAD REFEREE or a designated route official, as **G304** says. | The explanatory text must match the rules. |
...
| # | Change | Files |
|---|---|---|
| N10 | **Socket tube depth.** The 7.0-in tube length runs from the rim to the floor the CELL seats on, which is what makes a seated 14-in CELL stand 7.0 in proud (DESIGN-SPEC §3). The 0.09-in closed bottom lies beyond it, 7.09 in overall. Derived figures: Low and Mid Socket inboard edge 1.61 → **1.56 in**; Low Socket lowest point Z 22.27 → **22.19** (0.63 in above the tag target, was 0.71); Summit Socket inboard edge 2.96 → **2.94 in**; Summit Socket lowest point Z ≈ 64.4 → **64.29**, leaving **9.29 in** over a Shelf 2 CRATE (was 9.4). | CAD package §2.3, §2.4, §3, §7; vision guide §1.3; crag sheet; `verify/verify.py` |
| N11 | **BASECAMP clear volume.** A ROBOT in BASECAMP stands under the lower, wall-side face of the HEADWALL lane frames, not under the truss's field-side face. With the 2-in members that face lies at X = 41.788 − 0.268·Z, so H(X) = 3.7321 × (41.788 − X), reduced to 10.8 in under the lower crossbeam (X 34.2–38.6). A 42-in ROBOT fits up to **X = 30.5** (was published as 32.6). | CAD package §4.1; manual §3.2 and **G302** commentary; headwall sheet |
| N12 | **CRAG tag occlusion.** "No field structure occludes a CRAG tag" holds for a camera 10–20 in high. From the optional 24–36 in camera, the Low Socket tube can hide part of the tag beneath it (tags 8, 10, 21 and 23), and most of it from close range; the documents now say so. | spec §6; CAD package §7; vision guide §1.3 |
...
| N28 | **Repository reorganized** into `participants/`, the self-contained kit squads receive (manual, CADathon brief, field package with drawings and the Onshape Feature Studio, vision guide and layout), and `organizers/`, the materials used to create and run the challenge (`moderation/ORGANIZER-GUIDE.md`, `design/`, `source/` with every build and verification tool, and `archive/`). Organizer instructions moved out of the brief and the README into the organizer guide. The manual, the vision guide and the CAD package no longer defer to the design specification, which squads do not receive: the manual governs, and discrepancies are resolved by Team Update. Paths in rows A–N27 refer to the earlier layout. | Squads get exactly what they need, with nothing that belongs to the organizers mixed in. |

## O. Team Update TU-01 (Version 2.2)

Issued after the kickoff release; published to squads as `participants/05-team-updates/TEAM-UPDATE-01.md`, which gives every change in the struck/new format. No dimension, scoring value or rule number changed.

| # | Change | Files |
|---|---|---|
| O1 | SUPPLY compliance: CACHE CRATE compresses up to 2.0 in across opposing faces (13.0 → 11.0) under 15 lbf; ROPE COIL tube 2.5 → 2.0 under 10 lbf and ovalizes 10.0 → 9.0 (hole 5.0 → 4.0) under 5 lbf; both recover fully. Every fit is still computed at nominal size, so no clearance changes. | `DESIGN-SPEC.md`, section 02 (Table 3-4, §3.6), `FIELD-CAD-PACKAGE.md` §9, sheet 5 |
| O2 | O2 CELL made rigid: rigid molded ABS shell and caps replace the rigid core, EVA sleeve and foam caps. Mass and geometry unchanged. | `DESIGN-SPEC.md`, section 02, `FIELD-CAD-PACKAGE.md` §9.2, `MATERIALS-AND-COLORS.md`, sheet 5, `45_pieces.fs` label |
| O3 | **G203**: the "denting an O2 CELL cap" example (impossible on a rigid CELL) replaced by crushing a CACHE CRATE beyond its tolerance, and in-tolerance squeezing stated not to be alteration. | section 04 |
| O4 | Judging made optional: §1.1, **R303**, §7.4 and §8.5 no longer assume a judged event. The brief marks §5 (rubric) and §7 (awards) optional, applying only if the organizers announce judging at kickoff. | sections 01, 05, 06; `CADATHON-BRIEF.md`; `ORGANIZER-GUIDE.md` |
| O5 | Brief timeline: two weeks (Day 0 Sat to a Day 14 Sat deadline), four office hours (Days 3, 5, 10, 12), Team Update slots Days 4 and 9, design review Days 7–8, last call Day 11; optional judging Days 15–21 and results show Day ~22. | `CADATHON-BRIEF.md`, `ORGANIZER-GUIDE.md` |
| O6 | Figures re-rendered. `figures.py` had whitened every pixel matching the border's commonest colour; in the CENTER CACHE close-up that was the carpet, so the white tape vanished. It now flood-fills only background connected to the border. | `typesetting/pdf/figures.py`, `01-game-manual/figures/` |
| O7 | Drawings: FIELD centerline white tape (and its key swatch) drawn cased in `RULE_S`; CRAG View D socket labels spaced off their circles. | `generate_drawings.py`, sheets 1 and 2 |
| O8 | `verify/consist.py` built paths with `lstrip('./')`, which cut `.github` to `github` and crashed; it now uses `os.path.relpath`. | `verify/consist.py` |
| O9 | Kit additions: `participants/05-team-updates/`, and `organizers/discord/` (server setup and brand assets). | `participants/README.md`, `organizers/README.md`, `README.md` |
