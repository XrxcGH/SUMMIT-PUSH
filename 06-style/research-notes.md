# SUMMIT PUSH — Game Manual Typesetting Style Guide

**Version 1.0 · Internal production standard · Applies to all Markdown sources in this package**
...
This is the binding typographic and structural specification for the **SUMMIT PUSH Game Manual**, a fictional robotics-competition rulebook. It tells you exactly how to write the Markdown sources and exactly how to typeset them into a print-ready US Letter PDF.

The target look is the **genre convention** of a modern competitive-robotics game manual: a Word-processor-descended, single-column, print-first rulebook with hanging-indent rule numbering, color-coded rule headlines, tinted commentary boxes, demoted violation lines, and a running footer that carries section identity and revision state. That genre convention — the *structure* — is what we are matching.

### 1.2 What this document is emphatically not

We are **not** producing a counterfeit, a parody trading on someone else's marks, or a document that could be mistaken for an official publication of *FIRST*®, its FIRST Robotics Competition program, or any real organization. Section 10 states the exclusions in full and is normative, not advisory.
...
Facts about layout — that US Letter is 8.5 × 11 in, that a 0.5 in hanging indent reads well for numbered rules — are not anyone's intellectual property. Identity is. We take the former and invent the latter.
...
|---|---|---|
| Page size | **US Letter — 8.5 × 11 in (612 × 792 pt)** | Every page, including cover. |
...
The reference manual runs an unusually wide measure — a 7.5 in text column on an 8.5 in page. That is a Word artifact, and at 11 pt it produces roughly 105–115 characters per line, which is past the comfortable reading limit. We deliberately deviate.
...
> **Deviation noted.** The genre standard is a 0.50 in side margin / 7.50 in measure. We use 0.75 in / 7.00 in. This is a legibility improvement, it moves us visibly off an exact clone, and it costs roughly 4% more pages. If you must match the source geometry exactly for a comparison build, set `--measure-wide` in the build script; do not make it the default.
...
Note that the commentary box is inset deeper on *both* sides than any text indent. That double inset is what makes the box read as an aside without needing a heavy border.

### 2.4 Header

**A 1.19 in (86 pt) full-bleed band across the top of every page except the cover.**

Compose it as a **vector or flat-color band we author ourselves** — never a borrowed banner image:
...
**No running header text.** All wayfinding lives in the footer. This is a deliberate borrow — it keeps the header a constant graphic and lets the footer do the variable work.

### 2.5 Footer

Three variants. Baseline at **0.50 in from trim**, set in **Roboto Bold 10 pt** in `--ink`, spanning the full measure with three tab-stop cells (left / center / right).
...
- **Left cell:** literal string `Section N Title`, exactly as the H1 reads, including any parenthetical rule letter. Changes at each section boundary. This is the reader's primary wayfinding device.
...
**Adopt the per-section revision stamp.** The reference corpus shows two behaviors across years — a single document-wide stamp, and a per-section stamp where each section advertises its own revision level. The per-section behavior is strictly more useful to a reader, and the research explicitly flags it as the better design. **We use per-section stamps**: `Revision: S7-v4`. A reader scanning the footer learns instantly whether the section they are in has churned.
...
- Every top-level section **starts on a fresh page**, heading baseline at 1.47 in from trim (105.7 pt). Blank versos are acceptable; do not force even-page starts.
...
**The SUMMIT PUSH palette below is our own.** It is built to be *structurally analogous* — one deep accent that survives at 11 pt bold on white, one saturated stable-rule color, one light tint of the accent for boxes — while being visibly a different family. Where the reference sits in cyan-teal, we sit in **deep slate-indigo with a copper secondary**. Nobody will mistake one for the other; both do the same job.
...
> Two of the reference teals (`#0083AE` and `#0080A9`) differ by 3/255 and are a conversion artifact between two source applications, not a design decision. We do not reproduce that inconsistency: **one accent value, used everywhere.**
...
| `--accent-bright` | `#2E6FA8` | 46, 111, 168 | Header band divider rule, decorative rules only — **never body text** | 5.3:1 ⚠️ large text only |
...
| `--seasonal` | `#1F4E79` | 31, 78, 121 | **Season-specific rule headlines** (bold) — same as `--accent` by design | 9.4:1 ✅ |
...
**On the magenta leader color:** `#E100E1` appears nowhere else in the field palette — not in carpet gray, alliance red, alliance blue, or game-piece gold. That is the entire point. A callout can never be mistaken for field hardware. Keep it reserved; never use it for anything but leaders, arrowheads, and geometry-highlight strokes.
...
Roboto is used because it is the correct genre signal, is Apache-2.0 licensed, and is free for any use including commercial — its use is not a trademark issue. A typeface choice is not a brand asset.
...
**Space-before only. Zero space-after, everywhere, on every style except captions.**

This is the single most important typographic decision in the document and it is inherited directly from the genre. Every paragraph carries 6 pt above and 0 below. Consequences:
...
The **one exception**: figure/table captions carry 6 pt above and 10 pt below, because the caption sits above its object and needs to separate from it.

### 4.4 Below H2, hierarchy is carried by indentation, not size

H3, H4, H5, and H6 are **all 11 pt** — the same size as body text. They differ only by weight (H3 bold), style (H4 italic), color (H6 muted), and their position on the indent ladder. This is intentional and it is what keeps a 150-page rulebook from turning into a size-jump staircase. Do not "fix" it by scaling H3 up.
...
- **Non-breaking space** between a number and its unit (`24&nbsp;in`) and between a rule letter and its digits is unnecessary — rule IDs are single tokens (`G214`).
- **Small caps: never.** Defined terms are hard-typed capitals (§5.5).
- **Dimensions:** imperial first, metric in parentheses — `36 in (91 cm)`. State once in §1 that imperial governs and metric is convenience. Round metric so it stays rule-compliant: **maximums round down, minimums round up.**
...
Fifteen top-level sections. Each starts on a fresh page. Sections that carry rules append their rule letter, in parentheses, to the section title itself — this makes the footer self-documenting.
...
2. **Persistent rules occupy the low positions.** Every subsection places its persistent (cross-season) rules first, so `G201`–`G206` stay stable year over year and only the higher numbers churn.
...
4. A withdrawn rule's number is **retired, not reused.** Replace its body with a single line: `G217 Withdrawn.` in `--muted` italic.
...
| **Headline** | Roboto **Bold 11 pt**, `--stable` or `--seasonal` | A short colloquial sentence. **Bold, colored, NOT italic.** Ends with a period. Persistent rules prefix a literal `*`. |
...
> **Correction to a common misreading.** The headline is **bold colored, not italic**, and the rule *body* is **not bold** — only the number and the headline are. The only italic in a rule block is the Violation line. If you have seen a spec calling for italic headlines, it is wrong; do not follow it.
...
Note: the asterisk is **inside** the colored headline run, not a footnote marker, and there is no corresponding footnote anywhere.
...
**Violation lines are prose, not tables and not bullets.** Escalating consequences are written as a single running sentence, relying on ALL-CAPS defined terms to carry the structure. Never build a consequence table.
...
The word `Violation:` is **not** bolded and is **not** a separate run — the entire paragraph is uniformly gray italic. This visual demotion of the consequence below the rule is the single most distinctive typographic move in the genre. Keep it.

### 5.5 Defined terms

**Hard-typed ALL CAPS. No color, no weight change, no letterspacing, no small caps, no `text-transform`.**

Type `ROBOT`, not `Robot` wrapped in a caps class. The caps inherit the surrounding run's font, size, weight, and color exactly — so a defined term inside a bold headline is bold, inside a gray violation line is gray, inside a figure callout is whatever the callout is.

Rules:

1. Every ALL-CAPS term **must** have a §15 Glossary entry. Every glossary entry **must** appear in ALL CAPS somewhere in the body. Enforce with a build-time lint (§9.6).
2. Ordinary emphasis uses *italic*, never caps.
3. Acronyms that are not defined terms (`CAD`, `PDF`, `LED`) are caps but are **exempt** from the glossary requirement — maintain an allowlist in `build/acronyms.txt`.
4. Figure callout labels use the **identical** ALL-CAPS string as the glossary term, so a label and a body mention are visually and textually identical.
5. Do not pluralize a defined term by changing its case. `ROBOTS` is fine; `ROBOTs` is not.

### 5.6 Cross-references

**Format: `--link` `#245C9E`, underlined, in all three cases** — internal section links, rule references, and external URLs. Uniform treatment, no distinction.
...
| Glossary | `[CARGO](#g-cargo)` | <u>CARGO</u> — link the **first** occurrence per section only |
...
Anchors are contractual. Once published, an anchor never changes — other documents deep-link to them.

### 5.7 Commentary boxes

Our name for them, used in lowercase running prose: **note boxes**. (The genre calls its equivalent "blue boxes." We do not use that name — it is tied to a specific document's specific color. Ours are a different color and get a different name.)
...
**Design rule to preserve: the box border color always equals the season-specific headline color, and the fill is a light tint of it.** One accent drives both the headline and the callout chrome. Change the accent and the whole system re-themes coherently.

**Severity variants — a deliberate deviation.** The genre uses exactly one box for warnings, cautions, and notes, deliberately declining to color-code severity. We add two more, because SUMMIT PUSH includes physical-safety content where undifferentiated commentary is a real hazard:
...
> **Deviation noted.** The reference documents use a fully ruled grid (colored rules on every cell boundary) with **no** zebra striping. We use zebra striping with horizontal rules only and no verticals. This is a readability improvement for wide tables and a visible differentiator. The **glossary table** (§8.6) is the one place we keep the fully-ruled treatment.
...
The scoring table is the single most-read table in the manual. Build it as a **matrix crossing scoring action against game phase**, with thresholds broken out into a **separate** table.

**Table 6-4 · SUMMIT PUSH point values** — structure:
...
**Table 6-5 · SUMMIT PUSH bonus RP thresholds** — a separate table, columns `Regional & District Events | District Championships | Championship`, rows per RP. **Numeric thresholds never appear in the points table.** This separation is deliberate and load-bearing: it lets thresholds be revised by a Revision Note (§8.4) without touching the scoring *structure*, which is the part teams have memorized.
...
2. Drop table body to 9 pt (floor — never below 9 pt).
...
**Never** rotate a table 90°. **Never** insert a landscape page. **Never** shrink below 9 pt.
...
**Format: `Figure <section>-<n>`, sequence restarting at 1 in each section.** Figure 1-1, 1-2, 1-3, then Figure 5-1 … Figure 5-26, then 6-1, and so on. Tables use the identical scheme: `Table 6-4`.

Numbers are **generated by the build**, never hand-typed. Authors write `![](fig/arena-overview.png){#fig-arena-overview}` and reference `[Figure @fig-arena-overview]`; the filter assigns and renders the number.
...
| Separator | **`Figure 5-4 Title`** — space, no colon |
| Keep | Caption keeps-with-next; never orphaned from its object |

> **Deviation noted — we fix an inconsistency.** The reference documents use a space *and* a colon interchangeably, sometimes on the same page (`Figure 5-5 HUB` vs `Figure 5-6: HUB distance…`). **We standardize on the space, no colon, and lint for it.** Consistency is worth more than fidelity to someone else's typo.

**Caption content rules:**

- Repeat the defined term in ALL CAPS exactly as it appears in the glossary.
...
> **Deviation noted.** The reference figures are raster CAD screen captures at 114–223 dpi (median ~150), which do not survive zooming. **We require ≥200 dpi and prefer SVG** for anything we author. Our figures should be *better* than the ones we are modeling.
...
4. **Regions that are lines, not areas** (CENTER LINE, STARTING LINE) are **not** shaded — they get external callouts with leader lines instead.
...
| **Dot terminator** | Filled circle, same magenta — use when pointing at an **area**, not an edge |
| Label box | White fill, 0.75 pt `--ink` border, 3 pt padding, square corners |
| Label text | Roboto Bold 9 pt `--ink`, **ALL CAPS**, exactly matching the glossary term |
...
**Specification:** a rounded bar, fill `--ink`, height 0.22 in, containing `SCORING TABLE` in Roboto Bold 8 pt `--paper` with +1 pt letterspacing, flanked by a left-pointing and a right-pointing solid triangle. **Each arrow is filled in the alliance color of the end it points toward.** In SUMMIT PUSH, RED is on the left: red arrow left, blue arrow right. Hold this orientation across every figure in the manual — the banner is the field diagram's compass rose and its meaning depends on being invariant.
...
> **We do NOT reproduce the "PROPRIETARY AND CONFIDENTIAL / sole property of…" boilerplate.** It is inherited CAD-template text, it is self-contradictory on a publicly distributed build document, and reproducing it verbatim while naming a real organization would be a misrepresentation. If a notice is wanted, write an original one — or better, state the actual license (§10.5).

**Dimension line conventions:**

| Convention | Rule |
|---|---|
| **Reference dimensions** | **Parenthesized.** `(119.16)` — carries no tolerance. |
...
**Dates: ISO 8601 everywhere — `2026-02-26`.** Title block and revision table both. The reference documents mix US format in the title block with ISO in the revision table; that is an inconsistency, not a convention.
...
- **Two levels only** — section (`7`) and subsection (`7.2`). Third-level headings (`5.9.2`) exist in the body and are **deliberately excluded** from the TOC. This is the correct call: a three-level TOC for a 15-section rulebook becomes an index nobody reads.
...
4. **1.4** How to read this document — the conventions passage. This is where you state: defined terms are ALL CAPS and collected in the Glossary; cross-references and links appear in blue underlined text; note boxes explain but do not command and never override a rule; persistent rules are marked with a leading asterisk and a green headline while season-specific rules use a blue headline; imperial dimensions govern and metric is convenience; and the text means what it says with nothing hidden.
...
**Therefore, SUMMIT PUSH includes an in-document revision history**, placed in **front matter, on page 3 or 4, immediately after the Contents.**
...
- Both applied **inline within the reproduced paragraph**, so the reader sees the amended text in context rather than a before/after pair.
...
Alphabetize strictly by the term string. Cross-reference related terms with a linked `see also` at the end of a definition, in `--muted` italic.
...
| Option | Verdict |
|---|---|
| **Typst** | ✅ **Recommended.** Native, precise page geometry. Real running-header/footer logic with per-section state — which is exactly what our per-section revision stamp needs. Fast (sub-second rebuilds on a 150-page document). Modern scripting for rule numbering and figure numbering. Single self-contained binary, trivially CI-installable. Embeds fonts cleanly. |
...
**`style/tokens.yml` is the single source of truth for color.** Every template reads from it. A palette change is a one-file edit.
...
Author rules: authors type the rule ID and the headline. **They never type** the word `Violation:`, a figure number, a table number, or a page reference. Those are all generated.
...
- Print pages 1, 2, and one page from each rule-bearing section **in grayscale.** Every semantic distinction must still be readable.
- Confirm all fonts are embedded: `pdffonts out/SummitPush-GameManual.pdf` — expect Roboto, Roboto Condensed, JetBrains Mono, all subset-embedded, and **nothing else**.
...
### 10.1 Names and marks — never used
...
- Any sponsor name appearing in a real manual — including in a "presented by" construction, which is precisely the construction that implies sponsorship.
- Any real organization's name in a way that implies endorsement, affiliation, origin, or approval.

**We do not use "FRC-style" in any published-facing text.** It appears in this internal brief only. The shipped manual describes SUMMIT PUSH as a fictional competitive-robotics game, full stop.

### 10.2 Artwork and assets — never reproduced

- **No logos, wordmarks, or lockups** of any real organization, program, or sponsor — not in the header band, not on the cover, not in a drawing title block, not scaled down, not recolored, not partially cropped.
...
### 10.3 Verbatim text — never lifted
...
### 10.4 Colors — documented, not adopted
...
A single color is rarely protectable on its own, but a *palette* deployed in the *same layout* on a *similar document* is precisely how trade dress confusion is established. We change the palette so no such argument is available.
...
3. **Use freely-licensed fonts only** (Roboto, Roboto Condensed, JetBrains Mono — Apache 2.0 and OFL). Typeface choice is not a trademark issue; a typeface is a tool.
...
8. **Keep PDF metadata clean.** `Title: SUMMIT PUSH Game Manual`, `Author:` the project name, `Producer:` whatever the toolchain writes. No borrowed strings, ever.

### 10.6 The test to apply

Before shipping any page, ask: **if a stranger saw this page with no context, could they reasonably believe a real organization produced it?**
