...
for (const s of shots) {
  ...
  const info = await page.evaluate(o => window.stage(o), { ...s, models: models.map((_, i) => `/models/${i}.glb`) });
  const file = path.join(outDir, `${s.name}.png`);
  await page.locator('canvas').screenshot({ path: file });
  console.log(`${file}  ${info.triangles} triangles, ${info.calls} draw calls, ${((Date.now() - t0) / 1000).toFixed(1)} s`);
}
await browser.close();
server.close();
