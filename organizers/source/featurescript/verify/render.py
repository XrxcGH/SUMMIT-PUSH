...
Render the off-line build to PNG so the geometry can be inspected by eye.
    ...
    python organizers/source/featurescript/verify/render.py [--out DIR] [--lit]
...
def mesh_record(rec, defl=0.08):
    ...
    if t:
        ...
        h, s = t["cells"] / 2.0, t["s"]
        for (r, cc) in t["black"]:
            u0, v0 = (cc - h) * s, (h - r - 1) * s
            q = [o + x * u + y * v + n * 0.02 for (u, v) in ((u0, v0), (u0 + s, v0), (u0 + s, v0 + s), (u0, v0 + s))]
            tris += [[q[0], q[1], q[2]], [q[0], q[2], q[3]]]
            cols += [np.array((17, 17, 17)) / 255.0] * 2
