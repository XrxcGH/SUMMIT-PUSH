...
`build.py` concatenates `src/*.fs` in name order into `participants/03-field/onshape/SummitPushField.fs`, the file users paste
...
```
git clone https://github.com/javawizard/onshape-std-library-mirror organizers/source/featurescript/.fs-std   # once
python organizers/source/featurescript/build.py                 # assemble, lint, editor warnings, std API check
python organizers/source/featurescript/verify/run.py            # off-line build, self-check, palette, tags, SUPPLIES, interference
python organizers/source/featurescript/verify/run_checks.py     # the element checks (or name modules: run_checks.py headwall tags)
python organizers/source/featurescript/verify/render.py         # PNG views in renders/
```
