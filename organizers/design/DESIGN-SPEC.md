...
**Revision:** v2.2, superseding v2.1, v2.0 and v1.0. Every change since v1.0 is itemized, with rationale, in `organizers/design/REVISION-LOG.md` at the package root.
...
SUMMIT-PUSH/
  ...
  participants/                      — everything a CADathon squad receives
    README.md                        — start here: read order and how to use the kit
    01-game-manual/                  — SUMMIT-PUSH-Game-Manual.pdf, GAME-MANUAL.md, figures/
    02-cadathon/CADATHON-BRIEF.md    — event structure, timeline, deliverables, judging rubric
    03-field/                        — FIELD-CAD-PACKAGE.md (geometry + master ledger), MATERIALS-AND-COLORS.md,
                                       drawings/*.svg (6 sheets), onshape/SummitPushField.fs (field generator)
    04-vision/                       — VISION-GUIDE.md, apriltag-field-layout.json
  organizers/                        — for the people running the event; not part of the participant kit
    README.md                        — organizer map, building and verifying, making a change
    moderation/ORGANIZER-GUIDE.md    — running the event: release, Q&A, Team Updates, judging, results
    design/                          — DESIGN-SPEC.md (this file), REVISION-LOG.md
    source/manual/                   — manual sections, header, build.sh (compiles GAME-MANUAL.md)
    source/drawings/                 — generate_drawings.py, _drawlib.py (write the six sheets)
    source/featurescript/            — FeatureScript sources, build.py, off-line verification
    source/vision/make_layout.py     — generates the layout JSON from the vision guide's tag table
    source/typesetting/              — MANUAL-STYLE-GUIDE.md, research notes, pdf/ build, figure callouts
    source/verify/                   — document and drawing verification suite
    archive/                         — research/ and concepts/: the design-phase archive
```
