...
Finish the rendered figures: trim, size for the page, and place the callouts.
    ...
    python3 figures.py <render dir> <figure-shots.json> <out dir> <callout dir>
...
def finish(src, dst, shot):
    ...
    side = os.path.join(CALLOUT_DIR or dst, shot["name"] + ".callouts.json")
...
def main():
    ...
    CALLOUT_DIR = sys.argv[4] if len(sys.argv) > 4 else None
    if CALLOUT_DIR:
