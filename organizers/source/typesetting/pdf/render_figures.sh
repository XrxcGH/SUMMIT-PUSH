#!/usr/bin/env bash
# Render the manual's figures (participants/01-game-manual/figures/*.png) from the field model.
#
#   bash organizers/source/typesetting/pdf/render_figures.sh
#
# Builds the field off-line with the OpenCascade twin of the Onshape generator, exports it
# to glTF (organizers/source/featurescript/verify/export_glb.py) and renders the views in
# figure-shots.json with the headless three.js stage in organizers/source/featurescript/verify/webrender.
...
root="$(cd "$here/../../../.." && pwd)"
fs="$root/organizers/source/featurescript"
tmp="$here/build/figures"
mkdir -p "$tmp" "$root/participants/01-game-manual/figures"
...
python3 "$here/figures.py" "$tmp" "$here/figure-shots.json" "$root/participants/01-game-manual/figures" "$here/../figure-callouts"
