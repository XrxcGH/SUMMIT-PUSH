# -*- coding: utf-8 -*-
"""Layout check for the six generated drawing sheets (participants/03-field/drawings/*.svg).

The sheets print on landscape ANSI C plates with the image 21 in wide
(organizers/source/typesetting/pdf/plates.css), so one SVG user unit prints at 1512 / viewBox-width pt.
...
def main():
    ...
    fails = 0
    for f in sorted(glob.glob('participants/03-field/drawings/*.svg')):
