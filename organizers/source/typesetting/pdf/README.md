...
This folder typesets `organizers/source/manual/sections/*.md` into `participants/01-game-manual/SUMMIT-PUSH-Game-Manual.pdf`,
following `organizers/source/typesetting/MANUAL-STYLE-GUIDE.md`.

```bash
bash organizers/source/typesetting/pdf/build.sh           # the PDF
bash organizers/source/typesetting/pdf/render_figures.sh  # the figures in participants/01-game-manual/figures (after a field geometry change)
```
...
`organizers/source/featurescript/verify/webrender`. `figures.py` trims each image, projects each callout's
