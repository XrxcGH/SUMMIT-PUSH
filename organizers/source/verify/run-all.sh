...
bash organizers/source/manual/build.sh
python3 organizers/source/drawings/generate_drawings.py

fail=0
for f in verify.py consist.py crossdoc.py svg_collide.py svg_geom.py; do
  printf '\n================ %s ================\n' "$f"
  python3 "$here/$f" || fail=1
done
printf '\n================ make_layout.py --check ================\n'
python3 organizers/source/vision/make_layout.py --check || fail=1
printf '\n'
