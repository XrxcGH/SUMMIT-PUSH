...
During the event the organizing mentor is the game authority, the Q&A desk and the head referee, and takes no part in design. Judges are named at kickoff (brief §5.3). Mentors and alumni help only within the mentor-help rules in brief §3.2; an adult editing a squad's CAD document defeats the purpose of the event.
...
3. **Name the judges** (two or three, with a one-line background each) and **set the results show date**. Both are announced at kickoff, and the show date is kept (§5).
...
Run the kickoff show as the brief's timeline describes: the game reveal, a Game Manual walkthrough, and the release of the field CAD, drawings and AprilTag layout. Publish the brief and its judging rubric in full. The brief makes the rubric binding on the organizers (§5.3): it is final and public from Day 0, and no criteria are added, reweighted or reinterpreted afterwards. A public rubric helps rookie CAD teams set priorities and makes judging transparent; vague or hidden criteria are a common reason design challenges lose participants' trust. Open the Q&A desk.

## 4. During the build window (Days 1–8)

- **Strategy summaries (Day 2).** Reply to every posted summary with a short written check within 24 hours.
- **Office hours (Days 3, 4, 6).** Answer design questions under the mentor-help rules. Use them to find squads that are stuck and point them to the BASECAMP BOT path (brief Appendix A) while there is still time to change course.
- **Mid-event design review (Day 5).** A 15-minute slot per squad to walk through its in-progress CAD.
...
- **Team Updates.** Issue rule changes, and clarifications that change the manual text, only as numbered Team Updates, in the slots the brief sets (§6):
  ...
  3. Publish every update in the same channel. Each update amends the manual: the manual plus all updates is the current ruleset, and the most recent update governs where they conflict. The manual is not changed after Day 6 except to close a game-breaking exploit.
...
Judging delays and missing feedback are the two failures that most damage participants' trust in events like this. Both have occurred in community design challenges: results published a month after the promised date, and notices that individual feedback would not be provided, which removes one of the main reasons to enter. This event's judging window is one week, the show date is announced at kickoff, and written feedback for every squad is a required deliverable. If the organizers cannot staff that, reduce the size of the event, not the feedback.

## 6. Results show and after (Day ~15)

- Run the live, ranked feedback show: every submission shown, bottom-up reveal, verbal feedback on every entry, awards presented (brief §7). Deliver the written feedback the same night.
- Tally People's Choice from the peer ballots.
- Archive the Winner and Best Rookie Squad entries in the team's design library as reference examples.
