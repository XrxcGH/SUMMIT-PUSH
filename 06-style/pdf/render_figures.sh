...
python "$fs/verify/export_glb.py" --out "$tmp/field.glb"
...
python - "$here/figure-shots.json" "$tmp/shots.json" <<'PY'
import json, sys
...
python "$here/figures.py" "$tmp" "$here/figure-shots.json" "$root/02-manual/figures"
